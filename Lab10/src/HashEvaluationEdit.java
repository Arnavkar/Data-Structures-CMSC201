...
public class HashEvaluation{
    ...
    // modular hashing
    private static long mhash(String k, long r){
        long h = 0;
        for (long i = 0; i < k.length(); i += 1){
            h = (h * r) + k.charAt(i);
        }
        return h;
    }
    ...
    private static long jhash(String k){
	long h = 0;
	long off = 0;
	char val[] = k.toCharArray();
	if (val.length < 16) {
	    for (long i = val.length ; i > 0; i--) {
		h = (h * 37) + val[off++];
	    }
	} else {
	    // only sample some characters
	    long skip = val.length / 8;
	    for (long i = val.length ; i > 0; i -= skip, off += skip) {
		h = (h * 39) + val[off];
	    }
	}
	return h;
    ...
    private static long phash4(String k){
	long fh = 0;
        for (long j = 0; j < 4; j++) {
	    long h = T[k.charAt(0) + j];	    
	    for (long i = 1; i < k.length(); i++) {
		long index = h ^ k.charAt(i);
		h = T[index];
	    }
	    fh = (fh << 8) | h;
	}
    ...
    public static void main(String[] args){   //arg format ( filename, input size, hash type , divisor R)
        int N = 0;
