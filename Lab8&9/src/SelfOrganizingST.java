...
public class SelfOrganizingST<Key,Value extends Comparable<Value>> {
    ...
    public void put(Key key, Value val) { // Sequential put
        ...
        for (Node x = first; x != null; x = x.next) {
            if (key.equals(x.key)) { // if search hit
                this.delete(key); //delete the key from the list
                first = new Node(key, val, first); // add it with new value to the head of the list
                N++;
                return;
            }
        }
        first = new Node(key, val, first);// if search miss, create new node
        N++;
    }
